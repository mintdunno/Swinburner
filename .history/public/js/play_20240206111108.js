...
function setQuestion(questionObject) {
    ...
    setInterval(() => {
        for (int i = 1; i <= 4; i++) {
        container.option[i].innerHTML = `<input type="radio" name="answer" id="1" value="1" /><span class="option-color" id="option-color1">A:&nbsp;</span> ${option1} <span class="checked"></span>`;
    }
}, interval);
