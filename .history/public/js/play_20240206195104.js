...
//Oof Sound
var oofSound = document.getElementsByClassName("answer");
oofSound.forEach(element => {
    for (let i = 0, len = oofSound.length; i < len; i++) {
        oofSound[i].
    }
});
...
buttons.quit.addEventListener('click', () => {
    ...
    message.innerHTML = `Are you sure you want to quit?<br />You will win Rs ${slots[slot - 1]
        }`;
...
function setQuestion(questionObject) {
    ...
    setTimeout(() => {
        container.option4.innerHTML = `<input type="radio" name="answer" id="4" value="4" /><span class="option-color" id="option-color3">C:&nbsp;</span> ${option4} <span class="checked"></span>`;
